...
<?php if ((substr($_GET['bankid'],0,2))=='01'){	echo "<td class='c4'><b>รายการถอน</td>";
}
?>
...
if ((substr($_GET['bankid'],0,2))=='01'){
		$sql="select * from eventbank where bankid ='".$_GET['bankid']." '  and (time='1' or time='2') and (code='wi' or code='de') order by createdate";
}else{
		$sql="select * from eventbank where bankid ='".$_GET['bankid']." ' and code='de' and (time='1' or time='0') and code='de' order by createdate";
}
	$query= mysqli_query($link,$sql);
...
if ((substr($_GET['bankid'],0,2))=='01'){
	if(($result['time'])=='1'){
	echo "<td class='c1'>".$result['income']."</td>";
...
?>
	...
	<td class="c2"><?php if ($result['time']=='1'){$etime='คิดดอกเบี้ย';}else{if($result['time']=='0'){$etime='ถอนแล้ว';}else{$etime=$result['time'];}} ;
	 echo $etime; ?> </td>
</tr>
<?php
...
if ($sw==1){
/*
----//ตรวจสอบว่า ต้องเข้าไปทำรายการในเมนูใด  01 - สะสมทรัพย์ *******ยคิดวิธีจัดการ  ***********


*/
	include 'setinter.php';
	$em=substr($endtime,3,2);		$ey=substr($endtime,-2,2);   	$ed=substr($endtime,0,2);
	if ($result['time']==1){	$s=((($yn-$y)*4)+floor(($mn-$m)/3));	// ดูว่า ยอดนั้นหมดอายุหรือยัง
		} else {				$s=(($ey-$y)*4)+floor(($em-$m)/3);	}
$x=1;$a=1;$n=1;
if(empty($dateole)){$date1=substr(($result['createdate']),0,10);}else{$date1=$dateole;}
	$date2=substr(($result['createdate']),0,10);
	$cd=(strtotime($date2)-strtotime($date1))/( 60 * 60 * 24 );
// echo	$cd." และ ".$sumiin."=";
echo "<td>".$c."</td>";



echo "<td>".$result['deptime']."</td><td>	".$sinter=number_format(((($sumiin*$iin)/365)*$cd),2,'.',',');
...
echo "</td></tr>";
	...
	echo "<input type='hidden' name='time".$c."' value='1'>" ;
	echo "<input type='hidden' name='deptime".$c."' value='".$result['deptime']."'>" ;
...
/*
----ตรวจสอบว่า ต้องเข้าไปทำรายการในเมนูใด  02 - ประจำ 3 เดือน
-----การคิดงวดและใส่ปี ไม่ถูกต้อง  13/3/59
...
/*
-----ตรวจสอบว่า ต้องเข้าไปทำรายการในเมนูใด  03 - ประจำ 6 เดือน
...
*/
	...
	do {
	if ($a<=4){
		$sinter = number_format(($inter*$iin),3,'.',',')	;
	...
	echo "<tr><td>".$c."</td>
		<td>".date("d")."/".sprintf("%02d",$mt3[$a])."/".$y."</td>
					<td>".$sinter."</td>
					<td></td>
					<td>".number_format($inter=$inter+($inter*$iin),3,'.',',')."</td>
					<td>".$result['deptime']."-".sprintf("%03d",$n)."</td>
			</tr>";
		$a++;$n++;$c++;
		}else{// กรณีนี้ มีไว้เพื่อทำอย่างไร .....................................
