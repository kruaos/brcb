...
$workno=$_SESSION['workno'];
	$deptime = date('d')."_".date('m')."%";
	$sql="SELECT * FROM eventbank where workno=$workno and bankid like '0%' and deptime like '$deptime' order by createdate";
	$query=mysqli_query($link,$sql);
...
?></font></td>
	...
	<td class="c1"><font color="<?=$code;?>"><?php
	if ($dbarr['code']=='de'){echo number_format($result["income"],2,'.',',');}else{echo "-";};?></font></td>
	<td class="c2"><font color="<?=$code;?>"><?php
	if ($dbarr['code']=='wi'){echo number_format($result["income"],2,'.',',');}else{echo "-";};?></font></td>
...
<?php
	...
	$sql2="SELECT sum(income) as 'sincomde' FROM eventbank where code='de' and workno=$workno and deptime like '$deptime'  order by createdate";
	$query2=mysqli_query($link,$sql2);
	$sumd=mysqli_fetch_array($query2);	 
	$sql3="SELECT sum(income) as 'sincomwi' FROM eventbank where code='wi' and workno=$workno and deptime like'$deptime' order by createdate";
...
?>
<a href='../bank/printsend.php'>ปริ้นสรุปส่งเงิน</a>
</div></div>